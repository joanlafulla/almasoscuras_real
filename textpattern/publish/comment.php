...
$HeadURL: https://textpattern.googlecode.com/svn/releases/4.4.1/source/textpattern/publish/comment.php $
$LastChangedRevision: 3395 $
...
// -------------------------------------------------------------
	...
	function commentForm($id, $atts=NULL)
	{
		...
		$comment_submit_button = fInput('submit', 'submit', gTxt('submit'), 'button disabled button-disabled', '', '', '', '', 'txpCommentSubmit', true);

		// if all fields checkout, the submit button is active/clickable
		if ($preview) {
			$comment_submit_button = fInput('submit', 'submit', gTxt('submit'), 'button button-primary--small', '', '', '', '', 'txpCommentSubmit', false);
		}
		...
		else
		{
			...
			$checkbox = checkbox('remember', 1, $remember, '', 'remember').' '.tag(gTxt('remember'), 'label', ' for="remember" class="label-inline"');
		}

		$checkbox .= ' '.hInput('checkbox_type', $checkbox_type);

		$vals = array(
			'comment_name_input'		=> fInput('text', 'name', htmlspecialchars($name), 'comment_name_input comments-input comments-required'.($namewarn ? ' comments_error' : ''), '', '', $isize, '', 'name'),
			'comment_email_input'		=> fInput('text', 'email', htmlspecialchars($email), 'comment_email_input comments-input'.($emailwarn ? ' comments_error' : ''), '', '', $isize, '', 'email'),
			'comment_web_input'			=> fInput('text', 'web', htmlspecialchars($web)	, 'comment_web_input comments-input', '', '', $isize, '', 'web'),
			'comment_message_input' 	=> $textarea.'<!-- plugin-place-holder -->',
			'comment_remember'			=> $checkbox,
			'comment_preview'			=> fInput('submit', 'preview', gTxt('preview'), 'button button-primary--small', '', '', '', '', 'txpCommentPreview', false),
